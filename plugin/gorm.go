	...
	for _, schemaType := range cfg.Schema.Types {
		...
		case ast.Object, ast.InputObject:
			...
			for _, field := range schemaType.Fields {
				...
				ref := ""
				tag := `json:"` + field.Name + `"`
				if directive := field.Directives.ForName("gorm"); directive != nil {
					arg := directive.Arguments.ForName("tag")
					if arg != nil {
						tag = fmt.Sprintf(`%s gorm:"%s"`, tag, arg.Value.Raw)
					}
					arg = directive.Arguments.ForName("ref")
					if arg != nil {
						ref = fmt.Sprintf("%s `json:\"-\"`", arg.Value.Raw)
					}
				}
...
func (r *Plugin) InjectSourceEarly() *ast.Source {
	return &ast.Source{
		Name: "plugin/directives.graphql",
		Input: `
			directive @goField(
